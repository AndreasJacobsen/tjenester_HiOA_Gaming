...
if (isset($_POST['submit'])) {
 ...
 if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    $dateYearSecondSemester = $dateYearStart + 1; 
    $dateFirst = $dateYearStart . "-01-01";
    $dateFirstYear = $dateYearStart . "-08-01";
    
    $semesterDateCurrent = date("Y-m-d"); 
    $semesterOne = $dateYearStart . "-08-01";
    $semesterTwo = $dateYearSecondSemester . "-06-01";
    $semesterOneEnd = $dateYearStart . "-12-28";
    $semesterTwoEnd = $dateYearSecondSemester . "-07-28";

 
    
    if($semesterDateCurrent >= $semesterOne && $semesterDateCurrent < $semesterTwo) {
        $dateSemester = $semesterOneEnd;
        echo "semester en";
        echo $dateSemester;
    }
    elseif($semesterDateCurrent >= $semesterTwo && $semesterDateCurrent > $semesterOne){
        $dateSemester = $semesterTwoEnd;
        echo "semester to";
        echo $dateSemester;
    }
    else{
        echo "Error in registrering semester dates, please contact system admin or a HiOA Gaming programmer to resolve this issue";
        echo $dateSemester . "<br>" . $semesterDateCurrent . "<br>" . "<br>" .  $semesterOne . "<br>" . $semesterTwo;   
        $x++; 
    }

    //Test to se if we are in the first semester.
    if($semesterDateCurrent >= $semesterOne && $semesterDateCurrent < $semesterTwo){
        $dateYear = $dateYearSecondSemester . "-07-28";
        echo "<p>" . $dateYear;
    } 
    //test to see if we are in the second semester.
    elseif($semesterDateCurrent >= $semesterTwo && $semesterDateCurrent > $semesterOne){
        $dateYear = $dateYearStart . "-12-28";
        echo "<p>" . $dateYear;
    }
    else{
       echo "Error in registrering semester dates, please contact system admin or a HiOA Gaming programmer to resolve this issue";
...
*/
    ...
    if ($x === 0) {
        $sql = "INSERT INTO members (first_name, last_name, birth_date, student, gender, join_date, member_type, status, end_date, payment, email)
        VALUES ('$first_name','$last_name','$bday','$student','$gender','$date', '$payment', 'Member', '$end_date','$payGet','$email')";
